...
Herizon is a comprehensive women's health and wellness platform built with modern web technologies. The platform aims to provide a supportive environment for women to access health resources, connect with healthcare professionals, and engage with a community of like-minded individuals.
...
```bash
git clone https://github.com/yourusername/Herizon.git
cd Herizon
````
...
VITE_SERVER_URL=https://Herizon.onrender.com/
...
**For Frontend**:
```bash
    docker build -t Herizon-frontend .
```
- to run Frontend:
```bash
    docker run -it -p 5173:5173 Herizon-frontend
```

**For Backend** (You have to be in Backend folder) :
```bash
    docker build -t Herizon-backend .
```
- to run Backend:
```bash
    docker run -it -p 3000:3000 Herizon-backend
```
